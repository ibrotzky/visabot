...
<header class="dashboard_header">
    <div class="dashboard_header_title">
        <h1 class="icon-folder-minus">Fora de Estoque ou Inativos</h1>
...
<div class="dashboard_content">
    ...
    $Pager = new Pager("dashboard.php?wc=products/outsale&page=", "<<", ">>", 5);
    $Pager->ExePager($Page, 12);
    $Read->ExeRead(DB_PDT, "WHERE pdt_inventory <= 0 OR pdt_status != 1 ORDER BY pdt_status DESC, pdt_created DESC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
    if (!$Read->getResult()):
        $Pager->ReturnPage();
        echo Erro("<span class='al_center icon-notification'>Tudo certo {$Admin['user_name']}. Não existem produtos fora de estoque ou inativos!</span>", E_USER_NOTICE);
    else:
        ...
        $Pager->ExePaginator(DB_PDT, "WHERE pdt_inventory = 0 OR pdt_inventory IS NULL OR pdt_status != 1");
